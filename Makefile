...
generate: ## Regenerate sqlc + oapi-codegen + orval clients
	$(COMPOSE) run --rm --no-deps api sh -c "sqlc generate && cd internal/server/oapi && go run github.com/oapi-codegen/oapi-codegen/v2/cmd/oapi-codegen@v2.8.0 --config=config.yaml /workspace/api/openapi.yaml"
